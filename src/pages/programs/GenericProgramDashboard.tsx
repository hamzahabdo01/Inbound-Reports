...
const PROGRAM_KPIS: Record<string, { soh: string; issued: string; planned: string; git: string; expired: string; nExpiry: string }> = {
  'CH':  { soh: '2.5M ETB',   issued: '3.8M ETB',   planned: '966.4K ETB', git: '0',        expired: '47.9K ETB',  nExpiry: '4.6K ETB' },
  'DRI': { soh: '900.5K ETB', issued: '4.2M ETB',   planned: '1.9M ETB',   git: '0',        expired: '2.5K ETB',   nExpiry: '0' },
  'FP':  { soh: '0',          issued: '0',          planned: '0',          git: '0',        expired: '0',          nExpiry: '0' },
  'AD':  { soh: '0',          issued: '0',          planned: '0',          git: '0',        expired: '0',          nExpiry: '0' },
  'PED': { soh: '374.9K ETB', issued: '691.5K ETB', planned: '0',          git: '0',        expired: '610.2 ETB',  nExpiry: '3.8K ETB' },
  'VCT': { soh: '393.8K ETB', issued: '1.6K ETB',   planned: '0',          git: '5.5M ETB', expired: '11.3K ETB',  nExpiry: '0' },
  'LEP': { soh: '142.1K ETB', issued: '179.1K ETB', planned: '0',          git: '0',        expired: '1.2K ETB',   nExpiry: '0' },
  'MAL': { soh: '4.8M ETB',   issued: '9.4M ETB',   planned: '0',          git: '0',        expired: '13.8 ETB',   nExpiry: '249.8 ETB' },
  'MH':  { soh: '4.8M ETB',   issued: '5.8M ETB',   planned: '1.1M ETB',   git: '11K ETB',  expired: '574.1K ETB', nExpiry: '2.8K ETB' },
  'MD':  { soh: '1.1M ETB',   issued: '1.1M ETB',   planned: '0',          git: '0',        expired: '20.1K ETB',  nExpiry: '3.5K ETB' },
  'NTD': { soh: '16.7M ETB',  issued: '114.4M ETB', planned: '538.4K ETB', git: '0',        expired: '106.2K ETB', nExpiry: '473.7K ETB' },
  'Nu':  { soh: '5.9M ETB',   issued: '4.6M ETB',   planned: '4.5M ETB',   git: '0',        expired: '299.1K ETB', nExpiry: '3.3K ETB' },
  'RDF': { soh: '—',          issued: '—',          planned: '—',          git: '—',        expired: '—',          nExpiry: '—' },
  'TB':  { soh: '3M ETB',     issued: '6.3M ETB',   planned: '0',          git: '0',        expired: '16.9K ETB',  nExpiry: '1.8K ETB' },
  'VAC': { soh: '21.6M ETB',  issued: '75.1M ETB',  planned: '0',          git: '0',        expired: '80K ETB',    nExpiry: '3.8M ETB' },
};

interface GenericProgramDashboardProps {
  programCode: string;
  programName: string;
}

function GenericProgramDashboard({ programCode, programName }: GenericProgramDashboardProps) {
  const kpiData = PROGRAM_KPIS[programCode] || { soh: '0', issued: '0', planned: '0', git: '0', expired: '0', nExpiry: '0' };
  ...
  const fetchProcurerChart = useCallback(async (year) => {
    ...
    try {
      const res = await POD_WebApi.getItemProcurer({
        ModeCode: 'HPR', ProgramCode: programCode, FiscalYear: year,
      })
  ...
  const fetchTier1 = useCallback(async () => {
    ...
    try {
      const res = await IUBVIUI_WebApi.getProgramProducts({
        ModeCode: 'HPR', EnvironmentTypeCode: 'PFSAH', OrderBy: 'ProductCN', ProgramCode: programCode,
      });
    ...
    const timer = setTimeout(() => {
      ...
      stagger(0, () => {
        SS_WebApi.getEnvironmentSOHByDate({
          ModeCode: 'HPR', EnvironmentGroupCode: 'HUB', ProgramCode: programCode,
      ...
      stagger(50, () => {
        POD_WebApi.getItemProcurerPerItem({
          FiscalYear: '2016', ModeCode: 'HPR', ProgramCode: programCode, OrderBy: 'ProductCN',
        }).then(res => {
          const d = res?.data?.Data || [];
      ...
      stagger(100, () => {
        MainDashboard_WebApi.getReceiveTrend({
          ModeCode: 'HPR', EnvironmentCode: 'CNPH', ProgramCode: programCode,
        }).then(res => {
          const d = res?.data?.Data || [];
      ...
      stagger(200, () => {
        POHRIHRCH_WebApi.getCenterPipeline3({
          ModeCode: 'HPR', ProgramCode: programCode, OrderBy: 'ProductCN',
        }).then(res => {
          const d = res?.data?.Data || [];
      ...
      stagger(250, () => {
        FR_WebApi.getOrderFillRate({ ModeCode: 'HPR', ProgramCode: programCode }).then(res => {
          const d = res?.data?.Data || [];
      ...
      stagger(350, () => {
        SS_WebApi.getMOSShareBySite({
          ModeCode: 'HPR', ProgramCode: programCode, OrderBy: 'ProductCN', EnvironmentGroupCode: 'HUB',
        }).then(res => {
          const d = res?.data?.Data || [];
      ...
      stagger(400, () => {
        RCD_WebApi.getProgramManufacturer({
          ModeCode: 'HPR', FiscalYear: '2016', OrderBy: 'Manufacturer', ProgramCode: programCode,
        }).then(res => {
          const d = res?.data?.Data || [];
      ...
      stagger(450, () => {
        RCD_WebApi.getItemCountry({
          ModeCode: 'HPR', FiscalYear: '2016', ProgramCode: programCode,
        }).then(res => {
          const d = res?.data?.Data || [];
  ...
  const fetchIssuedItems = useCallback(async (from, to) => {
    ...
    try {
      const res = await OIH_WebApi.getItemDistributionHub2Facility({
        ModeCode: 'HPR', PageSize: 100, Page: 1, ProgramCode: programCode, From: from, To: to,
      });
  ...
  const refreshOrderFillRate = withRefresh('orderFillRate', async () => {
    const res = await FR_WebApi.getOrderFillRate({
      ModeCode: 'HPR', ProgramCode: programCode,
    });
  ...
  const refreshIssuedItems = withRefresh('issued', async () => {
    ...
    const res = await OIH_WebApi.getItemDistributionHub2Facility({
      ModeCode: 'HPR', PageSize: 100, Page: 1, ProgramCode: programCode, From: issuedFromDate, To: issuedToDate,
    });
    setApiIssuedItems(processIssuedItems(res?.data?.Data || []));
  });
  ...
  const refreshProcurer = withRefresh('procurer', async () => {
    const res = await POD_WebApi.getItemProcurerPerItem({
      FiscalYear: yearFilter, ModeCode: 'HPR', ProgramCode: programCode, OrderBy: 'ProductCN',
    });
  ...
  const refreshProcurerChart = withRefresh('procurerChart', async () => {
    const res = await POD_WebApi.getItemProcurer({
      ModeCode: 'HPR', ProgramCode: programCode, FiscalYear: yearFilter,
    });
    setApiProcurerChart(res?.data?.Data || []);
  });
  ...
  const refreshOwnershipChart = withRefresh('ownership', async () => {
    const res = await OIH_WebApi.getDistributionByOwnershipType({
      ModeCode: 'HPR', ProgramCode: programCode, FiscalYear: ownershipYear,
    });
    setApiOwnership(res?.data?.Data || []);
  });
  ...
  const refreshFundingChart = withRefresh('funding', async () => {
    const res = await POD_WebApi.getItemFundingSourceAndProcurer({
      ModeCode: 'HPR', ProgramCode: programCode, FiscalYear: fundingYear,
    });
    setApiFundingSource(res?.data?.Data || []);
  });
  ...
  const products = useMemo(() => stockRows.map((r: any) => r.ProductCN).sort(), [stockRows]);

  const filteredStock = useMemo(() => (
    stockRows.filter((r: any) => (
  ...
  const kpis = useMemo(() => ({
    ...
    sohAmt:         stockRows.reduce((s, r: any) => s + (r.SOHAmtBirr || 0), 0),
    issuedAmt:      stockRows.reduce((s, r: any) => s + (r.IssuedAmtBirr || 0), 0),
    expiredAmt:     stockRows.reduce((s, r: any) => s + (r.ExpiredAmtBirr || 0), 0),
    nearExpiryAmt:  stockRows.filter((r: any) => r.MOS > 0 && r.MOS < 3)
                      .reduce((s, r: any) => s + (r.SOHAmtBirr || 0), 0),
    atRisk:         stockRows.filter((r: any) => r.SS === 'Below EOP' || r.SS === 'Stocked Out' || r.SS === 'Excess').length,
    git:            stockRows.reduce((s, r: any) => s + r.GIT, 0),
    poQuantity:     purchaseOrders.reduce((s, r: any) => s + r.NextDeliveryQuantity, 0),
    receivedValue:  receives.reduce((s, r: any) => s + r.AmountReceivedBirr, 0),
  ...
  const stockUtilization = useMemo(() => {
    ...
    return data.slice(0, 14).map((row: any) => ({
  ...
  const selectedStockRow = useMemo(
    () => stockRows.find((row: any) => row.ProductCN === selectedProduct),
        ...
        <ProgramChartRow
          ...
          rightData={(() => {
            ...
            const totals = types.map((t) => ({
              ...
              value: apiDistribution.reduce((s: any, r: any) => s + (r[t] || 0), 0),
            }));
...
export default GenericProgramDashboard;
