...
export default function InfoModal({ contentId, isOpen, onClose }: any) {
  if (!isOpen) return null;

  const metadata: any = TABLE_METADATA[contentId] || {
    title: 'Guide',
          ...
          {metadata.example && metadata.example.columns && Object.keys(metadata.example.columns).length > 0 && (
            <section>
              ...
              <div className="border border-outline-variant rounded-xl overflow-hidden">
                {(Object.entries(metadata.example.columns) as [string, any][]).map(([colName, colData], i) => (
                  <div
                    key={colName}
